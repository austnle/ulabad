...
FirstVisit = filter(finaldata, visitnumber == 1)
AgeatBaseline = mean(FirstVisit$Age.AV45)
AgeatBaselineSD = sd(FirstVisit$Age.AV45)
AgeatBaselineSE = std.error(FirstVisit$Age.AV45)
AgeatBaselineSE2 = sd(FirstVisit$Age.AV45)/sqrt(length(FirstVisit$Age.AV45))

#Female (Values in Table 1: 72 and 45%)
FilteredforFemale = filter(finaldata, Gender == 2)
Female = length(unique(FilteredforFemale$SID))
FemalePercent = Female / length(unique(finaldata$SID))

#APOE4+ (Values in Table 1: 32 and 15%)
APOE4Status = filter(finaldata, APOE4_NUM == 1)
APOE4Plus = length(unique(APOE4Status$SID))
APOE4PlusPercent = APOE4Plus / length(unique(finaldata$SID))

#Education, y (Value in Table 1: 16.7 ± 2.7)
Education = mean(FirstVisit$Edu..Yrs.)
EducationSD = sd(FirstVisit$Edu..Yrs.)
AgeatBaselineSE = std.error(FirstVisit$Edu..Yrs.)
AgeatBaselineSE2 = sd(FirstVisit$Edu..Yrs.)/sqrt(length(FirstVisit$Edu..Yrs.))

#Converted to MCI/AD (Values in Table 1: MCI = 12, AD = 2, Percent = 9.9%)
MCI = filter(More2PETscans, visitnumber != 1, Diag.AV45 == "LMCI")
ConvertedtoMCI = length(unique(MCI$SID))

AD = filter(More2PETscans, visitnumber != 1, Diag.AV45 == "AD")
ConvertedtoAD = length(unique(AD$SID))

ConvertedtoMCIorADPercent = (ConvertedtoMCI + ConvertedtoAD)/length(unique(finaldata$SID))

#Converted to florbetapir+ (Values in table 1: 13 and 9.2%)


#Memory Composite (Value in Table 1: 1.12 ± 0.58)
Memory = mean(FirstVisit$UW_MEM_AV45)
MemorySD = sd(FirstVisit$UW_MEM_AV45)
MemorySE = std.error(FirstVisit$UW_MEM_AV45)
MemorySE2 = sd(FirstVisit$UW_MEM_AV45)/sqrt(length(FirstVisit$UW_MEM_AV45))

#Executive Function Compostie (Value in Table 1: 1.00 ± 0.67))
ExecFunction = mean(FirstVisit$UW_EF_AV45)
ExecFunctionSD = sd(FirstVisit$UW_EF_AV45)
ExecFunctionSE = std.error(FirstVisit$UW_EF_AV45)
ExecFunctionSE2 = sd(FirstVisit$UW_EF_AV45)/sqrt(length(FirstVisit$UW_EF_AV45))
